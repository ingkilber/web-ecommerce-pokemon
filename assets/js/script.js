...
(() => {
 ...
 const decorateSingles = () => grid.querySelectorAll('.single').forEach(card => { const item = cards[card.querySelector('[data-add-single]').dataset.addSingle]; if (!item.image) return; const art = card.querySelector('.single-art'); art.innerHTML = `<img src="${encodeURI(item.image)}" alt="${item[0]}">`; art.dataset.preview = 'true'; });
